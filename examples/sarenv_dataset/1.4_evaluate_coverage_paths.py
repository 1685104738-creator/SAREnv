...
if __name__ == "__main__":
    log.info("--- Initializing the Search and Rescue Toolkit for Custom Area ---")

    data_dir = "sarenv_outputs/test_area_01"  # Path to your custom dataset directory
    ...
    log.info("--- Generating Path Heatmap Visualizations for Custom Area ---")

    # Create output directory for heatmap plots
    output_dir = Path("sarenv_outputs/test_area_01/evaluation_outputs")
    ...
    for size, env_data in evaluator.environments.items():
        ...
        for name, generator in evaluator.path_generators.items():
            ...
            output_file = output_dir / f"{name}_{size}_custom_area_heatmap.pdf"
    ...
    log.info("--- Path Heatmap Visualization Complete for Custom Area ---")