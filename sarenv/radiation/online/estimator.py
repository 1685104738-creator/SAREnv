...
class IncrementalRadiationGrid:
    ...
    def __init__(
        ...
        *,
        ...
        self._mean = np.full(grid.shape, np.nan, dtype=np.float64)
        self._weight_sum = np.zeros(grid.shape, dtype=np.float64)

    @property
    def mean(self) -> np.ndarray:
        """Return a read-only view; unobserved cells contain NaN."""
    ...
    def estimate_at(self, x_m: float, y_m: float) -> RadiationEstimate:
        """Return the estimate in the containing 1 m cell, or explicit unknown."""
        if not math.isfinite(x_m) or not math.isfinite(y_m):
            raise ValueError("Estimate query coordinates must be finite.")
        if not self.grid.contains(x_m, y_m):
            return self._unknown_estimate(x_m, y_m)
    ...
    def _unknown_estimate(self, x_m: float, y_m: float) -> RadiationEstimate:
        return RadiationEstimate(
            x_m=float(x_m),
            y_m=float(y_m),
            value=None,
