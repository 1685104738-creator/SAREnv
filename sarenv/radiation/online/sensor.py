"""Minimal scalar radiation sensor for physical platform-height observations."""
...
class NoiseFreeRadiationSensor:
    """Query one scalar truth value at the UAV platform altitude.

    The sensor deliberately has no altitude-correction or ground-equivalent
    option.  A caller flying at 50 m therefore receives only the independently
    forward-simulated 50 m truth value.
    """
    ...
    def measure(
        ...
        *,
        ...
        """Return a scalar observation at the supplied platform altitude."""
        ...
        measurement_height_m = float(platform_altitude_m)
        truth_value = float(self._truth_query(x_m, y_m, measurement_height_m))
        value = max(truth_value - self.background_value_to_subtract, 0.0)
        return RadiationMeasurement(
            x_m=float(x_m),
            y_m=float(y_m),
            platform_altitude_m=measurement_height_m,
            value_reference_height_m=measurement_height_m,
